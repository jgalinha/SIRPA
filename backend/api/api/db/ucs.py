...
class InscricoesCursos(Base):
    ...
    id_ano_curricular = Column(
        Integer, ForeignKey("ano_curricular.id_ano"), primary_key=True
    )
...
class Semestres(Base):
    ...
    id_ano_curricular = Column(
        Integer, ForeignKey("ano_curricular.id_ano"), nullable=False
    )
...
class AnoCurricular(Base):
    ...
    inscricoes_cursos = relationship(
        "InscricoesCursos", back_populates="ano_curricular"
    )
